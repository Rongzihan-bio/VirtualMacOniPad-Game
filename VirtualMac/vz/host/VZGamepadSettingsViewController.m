...
{
    [super viewDidLoad];
    self.title = VZGPText("Network Gamepad Relay");
    ...
    enabledText.text = VZGPText("Continuously forward controller state at 60 Hz");
    ...
    UIStackView *destination = [[[UIStackView alloc]
        initWithArrangedSubviews:@[self.addressField, self.portField]] autorelease];
    destination.axis = UILayoutConstraintAxisHorizontal;
    destination.spacing = 10;

    UIButton *save = [UIButton buttonWithType:UIButtonTypeSystem];
    [save setTitle:VZGPText("Save and Apply") forState:UIControlStateNormal];
    [save addTarget:self action:@selector(saveDestination:)
    ...
    if (@available(iOS 15.0, *)) {
        save.configuration = [UIButtonConfiguration filledButtonConfiguration];
        self.testButton.configuration =
            [UIButtonConfiguration borderedButtonConfiguration];
    }
    UIStackView *buttons = [[[UIStackView alloc]
        initWithArrangedSubviews:@[save, self.testButton]] autorelease];
    buttons.axis = UILayoutConstraintAxisHorizontal;
    buttons.distribution = UIStackViewDistributionFillEqually;
    buttons.spacing = 12;
    UILabel *hint = [self labelWithFont:[UIFont preferredFontForTextStyle:
        UIFontTextStyleFootnote] color:UIColor.secondaryLabelColor];
    hint.text = VZGPText("For Bridge, use the guest LAN IPv4. For NAT, use the guest private IPv4. Start the UDP receiver with sudo first.");
    [content addArrangedSubview:[self cardWithTitle:VZGPText("Connection")
        views:@[enabledRow, destination, buttons, hint]]];
...
- (void)refresh
{
    ...
    self.networkLabel.text = [NSString stringWithFormat:
        VZGPText("Destination: %@\nSent: %llu packets  ACKs: %llu\n%@%@%@"),
        [state[@"destination"] length] ? state[@"destination"] : VZGPText("Not set"),
        (unsigned long long)sent, (unsigned long long)acked, ack, error, message];
